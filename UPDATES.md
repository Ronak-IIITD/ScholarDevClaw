...
### 2026-04-10 (Ops acceleration: production env template file)

**Goal:** Make production bootstrap faster by giving operators a ready-to-copy env template that matches current deploy-check + compose expectations.

**Summary:** Added `docker/.env.production.template` with all required security/bridge/provider/monitoring variables and placeholder conventions, and updated deployment docs to recommend copying this template directly for production setup.

**What changed:**

- `docker/.env.production.template` (new)
  - Includes:
    - core API auth/confinement/CORS settings,
    - agent/core bridge settings,
    - OpenClaw/Convex/GitHub integration settings,
    - provider selection + provider-key slots,
    - monitoring/runtime settings.

- `DEPLOYMENT.md`
  - Added `cp docker/.env.production.template docker/.env` path in environment setup instructions.
  - Added template-based alternative in full production checklist.

**Verification:**

- ✅ `python scripts/docs_lint.py`

### 2026-04-10 (Week-3 rocket fuel: deploy-check command for env/provider preflight)

**Goal:** Add a single command operators can run to validate production env files (including provider connectivity assumptions) before deploy.

**Summary:** Added `scholardevclaw deploy-check` backed by a new deploy preflight module. It parses an env file, validates required production variables, checks bridge mode, verifies selected provider key presence, flags placeholder secrets, and returns actionable errors/warnings.

**What changed:**

- `core/src/scholardevclaw/deploy/preflight.py` (new)
  - Added env parser + production preflight engine.
  - Checks:
    - required deployment vars,
    - placeholder value detection,
    - bridge mode validity,
    - provider key compatibility with selected provider,
    - Grafana default password hard-stop.

- `core/src/scholardevclaw/cli.py`
  - Added `deploy-check` command with:
    - `--env-file` (default `docker/.env`)
    - `--output-json` for machine-readable checks.

- `core/tests/unit/test_deploy_preflight.py` (new)
  - Added pass/fail coverage for deploy preflight logic.

- `core/tests/unit/test_cli.py`
  - Added command dispatch test for `deploy-check`.
  - Added command behavior tests (JSON success and missing file failure).

- `DEPLOYMENT.md`
  - Added `scholardevclaw deploy-check --env-file docker/.env --output-json` to release verification commands.
