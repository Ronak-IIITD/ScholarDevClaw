...
5. **Python test job fixture**: added the same pinned nanoGPT clone to `python-test` — `test_repos/` is entirely untracked, so `test_nano_analyze` failed with `files_scanned=0` (and e2e silently skipped) on fresh runners. Clone uses the acceptance manifest's pinned commit; `cli demo` already self-clones, which is why the demo job passed.
6. **nanogpt-opt steps 1–3** (pushed earlier today): frozen 10 specs in `core/specs/` + `nano_specs.py` loader (607be98); Python-only `nano_analyze.py` AST analyzer + tests (ab08149); mapper `_alias_lookup` dead-`self.*` fix + Llama/nanoGPT aliases + tests (26b127b).
